...
const setBoolean = (key: 'railroadStrict', checked: any) => {
  (settings as any)[key] = !!checked;
};
...
const addFabricWidth = () => {
  fabricWidthDrafts.value = [...fabricWidthDrafts.value, ''];
  normalizeFabricWidths();
};
...
// Template event wrappers (typed as any to avoid Ionic event type friction)
const onNumberInput = (key: 'widthMargin' | 'easeAllowance' | 'rfSnapSeparation', e: any) => {
  setNumber(key, e?.detail?.value);
};
const onBooleanChange = (key: 'railroadStrict', e: any) => {
  setBoolean(key, e?.detail?.checked);
};
const onFullnessInput = (k: keyof RipplefoldFullnessMap, e: any) => {
  setFullness(k, e?.detail?.value);
};
const handleReset = () => {
  reset();
...
.advanced-settings :deep(ion-item) {
  --background: #f9fafb; 
  background-color: #f9fafb; 
}
.dark .advanced-settings :deep(ion-item) {
  --background: #0000; 
  background-color: #0000; 
}

/* Center the toggle */
.railroad-toggle-shell {
  display: flex;
...
.railroad-toggle-item::part(native) {
  background: #f9fafb;
  box-shadow: none;
}
.dark .railroad-toggle-item::part(native) {
  background: #0000;
  box-shadow: none;
}
.railroad-toggle-item::part(inner) {
  border: 0 !important;
  align-items: center;
}
</style>
